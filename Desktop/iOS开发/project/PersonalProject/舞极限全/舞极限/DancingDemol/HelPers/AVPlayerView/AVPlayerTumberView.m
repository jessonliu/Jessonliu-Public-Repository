...
@implementation AVPlayerTumberView
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    AVPlayer *player = [AVPlayerSingleTon sendDataSingleton].temPlayer;
    [player pause];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(nullable UIEvent *)event{
    ...
    // 计算出对应的视频前进的时间
    CGFloat second = addX * [AVPlayerSingleTon sendDataSingleton].videoSecond / (22 * kBaseScale - kMargin) + [AVPlayerSingleTon sendDataSingleton].currentTime;
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if ([AVPlayerSingleTon sendDataSingleton].playState) {
            [player play];
        }
